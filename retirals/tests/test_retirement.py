...
class RetirementPlannerTest(unittest.TestCase):
    ...
    def test_standard_projection(self):
        ...
        self.assertAlmostEqual(metrics["corpus_at_retirement"], 25867431.69, places=1)
        self.assertAlmostEqual(metrics["final_corpus"], 0.0, places=1)
        self.assertEqual(metrics["years_in_retirement"], 36)
        self.assertEqual(metrics["peak_age"], 53)
        ...
        self.assertAlmostEqual(retirement_row["closing"], 25415017.34, places=2)
    ...
    def test_mild_crash_scenario(self):
        ...
        expected_return3 = (row3["opening"] + row3["contribution"] - row3["withdrawal"]) * (inputs.allocation_equity * inputs.return_equity + inputs.allocation_debt * inputs.return_debt + inputs.allocation_arbitrage * inputs.return_arbitrage + inputs.allocation_reit * inputs.return_reit)
    ...
    def test_zero_return_scenario(self):
        ...
        expected_return_50 = (p_50["opening"] + p_50["contribution"] - p_50["withdrawal"]) * (inputs.allocation_equity * inputs.return_equity + inputs.allocation_debt * inputs.return_debt + inputs.allocation_arbitrage * inputs.return_arbitrage + inputs.allocation_reit * inputs.return_reit)
        self.assertAlmostEqual(p_50["return"], expected_return_50, places=2)
        self.assertAlmostEqual(p_50["closing"], p_50["opening"] + p_50["contribution"] - p_50["withdrawal"] + p_50["return"], places=1)
        p_51 = next(p for p in result["projections"] if p["age"] == 51)
        expected_return_51 = (p_51["opening"] + p_51["contribution"] - p_51["withdrawal"]) * (inputs.allocation_equity * inputs.return_equity + inputs.allocation_debt * inputs.return_debt + inputs.allocation_arbitrage * inputs.return_arbitrage + inputs.allocation_reit * inputs.return_reit)
        self.assertAlmostEqual(p_51["return"], expected_return_51, places=2)
        self.assertAlmostEqual(p_51["closing"], p_51["opening"] + p_51["contribution"] - p_51["withdrawal"] + p_51["return"], places=1)
        # Post-retirement (age 52, 53, 54)
        p_52 = next(p for p in result["projections"] if p["age"] == 52)
        expected_return_52 = (p_52["opening"] + p_52["contribution"] - p_52["withdrawal"]) * (inputs.allocation_equity * inputs.return_equity + inputs.allocation_debt * inputs.return_debt + inputs.allocation_arbitrage * inputs.return_arbitrage + inputs.allocation_reit * inputs.return_reit)
        self.assertAlmostEqual(p_52["return"], expected_return_52, places=2)
        self.assertAlmostEqual(p_52["closing"], p_52["opening"] + p_52["contribution"] - p_52["withdrawal"] + p_52["return"], places=1)
        p_53 = next(p for p in result["projections"] if p["age"] == 53)
        expected_return_53 = (p_53["opening"] + p_53["contribution"] - p_53["withdrawal"]) * (inputs.allocation_equity * inputs.return_equity + inputs.allocation_debt * inputs.return_debt + inputs.allocation_arbitrage * inputs.return_arbitrage + inputs.allocation_reit * inputs.return_reit)
        self.assertAlmostEqual(p_53["return"], expected_return_53, places=2)
        self.assertAlmostEqual(p_53["closing"], p_53["opening"] + p_53["contribution"] - p_53["withdrawal"] + p_53["return"], places=1)

        p_54 = next(p for p in result["projections"] if p["age"] == 54)
        expected_return_54 = (p_54["opening"] + p_54["contribution"] - p_54["withdrawal"]) * (inputs.allocation_equity * inputs.return_equity + inputs.allocation_debt * inputs.return_debt + inputs.allocation_arbitrage * inputs.return_arbitrage + inputs.allocation_reit * inputs.return_reit)
        self.assertAlmostEqual(p_54["return"], expected_return_54, places=2)
        self.assertAlmostEqual(p_54["closing"], p_54["opening"] + p_54["contribution"] - p_54["withdrawal"] + p_54["return"], places=1)
    ...
    def test_exhaustion_with_ltcg_exemption(self):
        ...
        self.assertAlmostEqual(exhaustion_row["withdrawal"], 55400.0, places=2, msg="Gross withdrawal should be capped to available corpus.")
        self.assertAlmostEqual(exhaustion_row["withdrawal_tax"], 4875.2, places=2, msg="Tax in exhaustion year must be recalculated respecting LTCG exemption.")
        self.assertAlmostEqual(exhaustion_row["withdrawal_after_tax"], 50524.8, places=2, msg="Net withdrawal must be recalculated.")
        self.assertAlmostEqual(exhaustion_row["closing"], 0.0, places=2)
...
class REITTaxTest(unittest.TestCase):
    def test_reit_gain_fraction_behavior(self):
        """Test REIT gain fraction effect on LTCG portion."""
        base_args = dict(
            current_age=60,
            retirement_age=65,
            life_expectancy=80,
            ...
            current_annual_expenses=1000.0,
            avg_inflation_rate=0.0,
            ...
            return_equity=0.0,
            return_debt=0.0,
            return_arbitrage=0.0,
            return_reit=0.0,
            allocation_equity=0.0,
            allocation_debt=0.0,  # will be set in loop
            allocation_arbitrage=0.0,
            ...
            one_time_lumpsum=0.0,
            stress_scenario=StressScenario.NORMAL,
            adhoc_expenses=[]
        )
        scenarios = [
            (0.0, 0.5, 0.0),
        ...
        for alloc_reit, gain_frac, expected_blended in scenarios:
            with self.subTest(allocation_reit=alloc_reit, reit_gain_fraction=gain_frac):
                args = base_args.copy()
                args["allocation_reit"] = alloc_reit
                args["allocation_debt"] = 1.0 - alloc_reit  # keep total allocation 100%
                args["reit_gain_fraction"] = gain_frac
                inputs = PlannerInputs(**args)
                result = run_projection(inputs)
                # Find the projection for the retirement age
                idx = inputs.retirement_age - inputs.current_age
                proj = result["projections"][idx]
                net_required = 1000.0
                if alloc_reit == 0.0 or gain_frac == 0.0:
                    expected_tax = 0.0
                    expected_withdrawal = net_required
                else:
                    expected_blended = gain_frac * alloc_reit * 0.125
                    expected_gross = net_required / (1.0 - expected_blended)
                    expected_tax = expected_gross - net_required
                    expected_withdrawal = expected_gross
                self.assertAlmostEqual(proj["withdrawal_tax"], expected_tax, places=2,
                                       msg=f"Tax mismatch for alloc_reit={alloc_reit}, gain_frac={gain_frac}")
                self.assertAlmostEqual(proj["withdrawal"], expected_withdrawal, places=2,
                                       msg=f"Withdrawal mismatch for alloc_reit={alloc_reit}, gain_frac={gain_frac}")
