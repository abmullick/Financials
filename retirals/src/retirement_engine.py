...
from return_model import calculate_portfolio_expected_return
...
def get_return_rate(age: int, inputs: PlannerInputs) -> float:
    ...
    if not is_retired:
        return calculate_portfolio_expected_return(inputs)
...
def run_projection(inputs: PlannerInputs):
    ...
    reit_ltcg_portion = inputs.allocation_reit * inputs.reit_gain_fraction
    total_ltcg_portion = equity_ltcg_portion + reit_ltcg_portion
...
def _calculate_gap_analysis(inputs: PlannerInputs, corpus_at_retirement: float, minimum_corpus_required: float) -> dict:
    ...
    if gap > 0:
        ...
        r = calculate_portfolio_expected_return(inputs)  # Annual return rate
...
def _solve_for_required_returns(inputs: PlannerInputs, corpus_at_retirement: float, minimum_corpus_required: float, ad_hoc_map: dict, one_time_income_map: dict | None = None) -> dict:
    ...
    """
    required_pre_ret_return = calculate_portfolio_expected_return(inputs)
    gap = minimum_corpus_required - corpus_at_retirement
    ...
    if gap <= 0:
        ...
        required_post_ret_return = calculate_portfolio_expected_return(inputs)
        def get_min_corpus(rate):
            ...
            return _calculate_minimum_corpus(inputs.model_copy(update={
                "return_equity": rate,
                "return_debt": rate,
...
def _get_mc_return(age: int, inputs: PlannerInputs, rng: random.Random) -> float:
    ...
    is_retired = age >= inputs.retirement_age
    base_return = calculate_portfolio_expected_return(inputs)
...
def _generate_recommendations(inputs: PlannerInputs, mc_summary: dict) -> list:
    ...
    current_portfolio_return = calculate_portfolio_expected_return(inputs)
    current_portfolio_return_pct = current_portfolio_return * 100.0
    ...
    if gap > 0:
        ...
        if 0 < req_pre_return <= 50.0 and req_pre_return > current_portfolio_return_pct + 1.0:
            recs.append(f"To reach your goal without extra savings, pre-retirement returns would need to be around {req_pre_return:.1f}% instead of {current_portfolio_return_pct:.1f}%.")
        
        if 0 < req_post_return <= 50.0 and req_post_return > current_portfolio_return_pct + 1.0:
            recs.append(f"Post-retirement returns would need to be around {req_post_return:.1f}% instead of {current_portfolio_return_pct:.1f}% to close the gap.")
