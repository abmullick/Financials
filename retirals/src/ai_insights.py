...
5. CASH-FLOW INTERPRETATION: Cover pension (timing, amount, tax, surplus reinvestment), ad-hoc expenses (timing, magnitude, impact), future one-time incomes (each income's timing, amount, inflation-adjusted effect, and contribution to funding/sustainability), the retirement lumpsum if present, and recurring retirement expenses (inflation impact). State when a feature is absent from the scenario rather than implying it does not exist for the user.
...
class CashFlowInsights(BaseModel):
    ...
    one_time_retirement_income: str = Field(..., description="Interpretation of future one-time incomes, including timing, inflation-adjusted funding effect, and retirement sustainability impact.")
...
def _build_compact_prompt(request: AIInsightRequest) -> str:
    ...
    compact_user_inputs = {
        ...
        "allocation_reit": user_inputs.allocation_reit,
        ...
        "volatility_reit": user_inputs.volatility_reit,
        ...
        "equity_reit_correlation": user_inputs.equity_reit_correlation,
        "debt_reit_correlation": user_inputs.debt_reit_correlation,
        "arbitrage_reit_correlation": user_inputs.arbitrage_reit_correlation,
        "return_equity": user_inputs.return_equity,
        "return_debt": user_inputs.return_debt,
        "return_arbitrage": user_inputs.return_arbitrage,
        "return_reit": user_inputs.return_reit,
        "reit_gain_fraction": user_inputs.reit_gain_fraction,
